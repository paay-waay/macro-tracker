/* Macro Tracker 3.1.0 — local-first UI. No analytics or remote requests. */
(() => {
'use strict';
...
function renderedEntry(e){return `${format(e.calories)} kcal · P ${format(e.protein)} · C ${format(e.carbs)} · F ${format(e.fat)}`;}
function macroDetail(e){return `<span class="p">P ${format(e.protein)}</span><span class="c">C ${format(e.carbs)}</span><span class="f">F ${format(e.fat)}</span>`;}
...
function todayHTML(){
 ...
 const recentSeen=new Set(quick.map(t=>t.name));
 const fallback=Object.values(data.days).sort((a,b)=>b.date.localeCompare(a.date)).flatMap(d=>d.entries).filter(e=>{if(recentSeen.has(e.name))return false;recentSeen.add(e.name);return true;}).slice(0,4);
 const shortcuts=quick.length?quick:fallback;
 const macroValue=k=>sum[k]===0&&!d.entries.length?'0':format(sum[k]);
 const missing=k=>d.entries.some(e=>C.num(e[k])===null);
 ...
 <section class="card daily-card" aria-label="每日进度">
  ...
  <div class="macro-grid">
   ...
   <div class="macro-cell carbs"><span><i></i>碳水</span><div><strong>${macroValue('carbs')}</strong><small>g</small></div><small>${missing('carbs')?'有数值待补':'已记录'}</small></div>
   <div class="macro-cell fat"><span><i></i>脂肪</span><div><strong>${macroValue('fat')}</strong><small>g</small></div><small>${missing('fat')?'有数值待补':'已记录'}</small></div>
  </div>
 </section>
 ${savedDraft()?'<button class="resume" data-action="resume">继续上次输入 <span>→</span></button>':''}
 ${shortcuts.length?`<section class="card favorite-card"><div class="section-heading"><h2>${quick.length?'常吃':'最近吃过'}</h2><button class="text-button" data-action="library">全部</button></div><div class="shortcuts">${shortcuts.map(t=>`<button class="shortcut" data-action="${quick.length?'quick':'recent'}" data-id="${escape(t.id)}" aria-label="添加${escape(t.name)}"><span>${escape(t.name)}</span><small>${format((quick.length?defaultPortion(t):t).calories)} kcal</small><b aria-hidden="true">＋</b></button>`).join('')}</div></section>`:''}
...
function historyRows(days){return days.map(d=>`<button data-action="date" data-date="${d.date}"><span><strong>${d.date}</strong><small>${d.entries.length} 条${d.weight?' · '+format(d.weight,.1)+' kg':''}</small></span><span>${d.entries.length?(C.totals(d.entries).missingCalories?'已知 ':'')+format(C.totals(d.entries).calories)+' kcal':'—'}<small>${d.coverage==='complete'?'已记全':''}</small></span></button>`).join('')||'<p class="empty">暂无记录</p>';}
...
function editorForm(e={},mode='new',saved=null){
 ...
 show(mode==='edit'?'编辑记录':mode==='template'?'编辑常吃':'记一餐',`<form id="food-form" class="stack"><div class="food-grid">${[['calories','热量','kcal'],['protein','蛋白质','g'],['carbs','碳水','g'],['fat','脂肪','g']].map(([key,label,unit])=>`<label class="nutrition-input ${key}"><span>${label}<small>${unit}</small></span><input aria-label="${label}" name="${key}" type="number" min="0" max="100000" step="any" inputmode="decimal" enterkeyhint="next" value="${e[key]??''}" placeholder="—"></label>`).join('')}</div><label class="name-label">名称 <span class="muted">可选</span><input name="name" maxlength="300" value="${escape(e.name||'')}" placeholder="例如：鸡肉饭" enterkeyhint="done"></label>${mode==='template'?'':'<label class="check save-favorite"><input name="saveTemplate" type="checkbox">存为常吃</label>'}<div id="form-error" class="error" role="alert"></div><div id="draft-status" class="error" role="status"></div><button class="primary wide submit-food" type="submit">${mode==='template'?'保存常吃':mode==='edit'?'保存修改':'记入'+(date===C.today()?'今天':date.slice(5))}</button><div class="editor-tools">${mode==='edit'?`<button type="button" class="danger" data-action="remove" data-id="${escape(e.id)}">删除记录</button>`:mode==='template'?`<button type="button" class="danger" data-action="delete-template" data-id="${escape(e.id)}">移出常吃</button>`:'<button type="button" class="quiet" data-action="discard-draft">清空输入</button>'}</div></form>`);
 ...
 $('food-form').onsubmit=async ev=>{ev.preventDefault();const f=new FormData(ev.target);const out={...C.entry(e),name:String(f.get('name')).trim(),source:e.source||'manual'};C.fields.forEach(k=>out[k]=C.num(f.get(k)));
  if(!out.name&&C.fields.every(k=>out[k]===null)){$('form-error').textContent='填入宏量即可记录';return;}
...
function library(query=''){
 ...
 const seen=new Set(data.templates.map(e=>[e.name,...C.fields.map(k=>defaultPortion(e)[k])].join('|')));const recent=Object.values(data.days).sort((a,b)=>b.date.localeCompare(a.date)).flatMap(d=>d.entries).filter(e=>{const key=[e.name,...C.fields.map(k=>e[k])].join('|');if(seen.has(key)||!matches(e))return false;seen.add(key);return true;}).slice(0,15);
 const row=(e,isTemplate)=>`<div class="library-row"><button class="library-add" data-action="${isTemplate?'quick':'recent'}" data-id="${escape(e.id)}" aria-label="添加${escape(e.name)}"><span><strong>${escape(e.name||'这餐')}</strong><small>${escape(renderedEntry(isTemplate?defaultPortion(e):e))}</small></span><b aria-hidden="true">＋</b></button>${isTemplate?`<button class="quiet library-edit" data-action="template-menu" data-id="${escape(e.id)}" aria-label="编辑${escape(e.name)}">编辑</button>`:''}</div>`;
 return `${templates.length?'<h3 class="list-heading">常吃</h3>'+templates.map(t=>row(t,true)).join(''):''}${recent.length?'<h3 class="list-heading">最近</h3>'+recent.map(t=>row(t,false)).join(''):''}${!templates.length&&!recent.length?'<p class="empty">暂无记录</p>':''}`;
}
function openLibrary(){show('常吃与最近',`<input id="library-search" type="search" placeholder="搜索名称" aria-label="搜索常吃与历史"><div class="list" id="library-list">${library()}</div>`);$('library-search').oninput=e=>$('library-list').innerHTML=library(e.target.value);}
...
function templateMenu(t){editorForm(defaultPortion(t),'template');}
...
document.addEventListener('click',async ev=>{
 ...
 if(action==='add')editorForm();else if(action==='new-entry')editorForm();else if(action==='edit')editorForm(day().entries.find(e=>e.id===id),'edit');else if(action==='library')openLibrary();
 else if(action==='quick'){const t=data.templates.find(t=>t.id===id);await addTemplate(t,(t.lastQuantity||t.quantity||1)/(t.quantity||1));}else if(action==='recent')await addTemplate(findEntry(id),1,false);else if(action==='template-menu')templateMenu(data.templates.find(t=>t.id===id));
 ...
 else if(action==='delete-template'){if(await commit(n=>n.templates=n.templates.filter(x=>x.id!==id),'已移出常吃')){clearDraft();close();render();}}
