/* Macro Tracker 3.2.2 — local-first UI. No analytics or remote requests. */
(() => {
'use strict';
...
function editorForm(e={},mode='new',saved=null){
 ...
 $('food-form').onsubmit=async ev=>{ev.preventDefault();const f=new FormData(ev.target);const out={...C.entry(e),name:String(f.get('name')||'').trim().slice(0,300)||'记录',source:e.source||'manual',calories:C.num(f.get('calories')),protein:C.num(f.get('protein'))};
 ...
 const inputs=[...$('food-form').querySelectorAll('input[type="number"]')];$('food-form').elements.namedItem('name').addEventListener('keydown',ev=>{if(ev.key==='Enter'){ev.preventDefault();inputs[0].focus();}});inputs[0].addEventListener('keydown',ev=>{if(ev.key==='Enter'){ev.preventDefault();inputs[1].focus();}});
